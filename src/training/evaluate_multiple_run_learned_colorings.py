"""
Evaluate learned GNN orderings from multiple training-run checkpoints.

For each saved checkpoint, this script:
1. loads the trained model,
2. predicts node scores on the test graph,
3. converts scores into a learned ordering,
4. applies greedy coloring,
5. stores the number of colors and validity.
"""

from __future__ import annotations

import csv
import time
from pathlib import Path

import torch

from src.graphs.sparse_matrix_to_graph import load_graph_from_mtx
from src.models.gnn_node_scorer import GNNNodeScorer
from src.training.learned_ordering import scores_to_ordering
from src.training.load_pyg_splits import load_all_pyg_graphs, group_dataset_by_split
from src.training.ordered_greedy_coloring import (
    count_colors,
    greedy_color_with_ordering,
    is_valid_coloring,
)


TRAINING_SUMMARY_CSV = Path(
    "results/tables/gnn_node_scorer/multiple_run_training_summary.csv"
)

OUTPUT_CSV = Path(
    "results/tables/gnn_node_scorer/multiple_run_learned_coloring_evaluation.csv"
)

TEST_MATRIX_PATHS = {
    "jac_pat": Path("data/raw/matrices/jac_pat.mtx"),
}


def load_training_run_rows() -> list[dict[str, str]]:
    """
    Load repeated training run rows.
    """

    if not TRAINING_SUMMARY_CSV.exists():
        raise FileNotFoundError(
            f"Training summary CSV not found: {TRAINING_SUMMARY_CSV}"
        )

    with TRAINING_SUMMARY_CSV.open(newline="", encoding="utf-8") as csv_file:
        reader = csv.DictReader(csv_file)
        rows = list(reader)

    if not rows:
        raise ValueError("No rows found in multiple-run training summary.")

    return rows


def evaluate_checkpoint(
    checkpoint_path: Path,
    graph,
    graph_data,
) -> tuple[int, bool, float]:
    """
    Evaluate one checkpoint on the test graph.

    Returns
    -------
    tuple[int, bool, float]
        num_colors, valid, runtime_seconds
    """

    checkpoint = torch.load(checkpoint_path, weights_only=False)

    model = GNNNodeScorer(
        in_channels=checkpoint["input_dim"],
        hidden_channels=checkpoint["hidden_channels"],
        out_channels=checkpoint["out_channels"],
    )
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()

    start_time = time.perf_counter()

    with torch.no_grad():
        predicted_scores = model(graph_data.x, graph_data.edge_index)

    learned_ordering = scores_to_ordering(predicted_scores)

    learned_coloring = greedy_color_with_ordering(
        graph=graph,
        ordering=learned_ordering,
    )

    runtime_seconds = time.perf_counter() - start_time

    num_colors = count_colors(learned_coloring)
    valid = is_valid_coloring(graph, learned_coloring)

    return num_colors, valid, runtime_seconds


def main() -> None:
    training_rows = load_training_run_rows()

    dataset = load_all_pyg_graphs()
    grouped = group_dataset_by_split(dataset)

    test_graph_data = grouped["test"][0]
    graph_id = test_graph_data.graph_id

    if graph_id not in TEST_MATRIX_PATHS:
        raise ValueError(f"No matrix path configured for test graph: {graph_id}")

    graph = load_graph_from_mtx(TEST_MATRIX_PATHS[graph_id])

    rows = []

    print("Multiple-run learned coloring evaluation")
    print("----------------------------------------")

    for training_row in training_rows:
        seed = training_row["seed"]
        checkpoint_path = Path(training_row["checkpoint_path"])

        num_colors, valid, runtime_seconds = evaluate_checkpoint(
            checkpoint_path=checkpoint_path,
            graph=graph,
            graph_data=test_graph_data,
        )

        result_row = {
            "graph_id": graph_id,
            "seed": seed,
            "method_family": "gnn",
            "method_name": "GNNNodeScorer",
            "ordering_name": "learned_ordering_from_predicted_scores",
            "num_vertices": graph.number_of_nodes(),
            "num_edges": graph.number_of_edges(),
            "num_colors": num_colors,
            "valid": valid,
            "runtime_seconds": runtime_seconds,
            "checkpoint_path": str(checkpoint_path),
        }

        rows.append(result_row)

        print(
            f"Seed {seed} | "
            f"colors: {num_colors} | "
            f"valid: {valid} | "
            f"runtime: {runtime_seconds:.6f}s"
        )

    OUTPUT_CSV.parent.mkdir(parents=True, exist_ok=True)

    with OUTPUT_CSV.open("w", newline="", encoding="utf-8") as csv_file:
        fieldnames = list(rows[0].keys())
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)

    print()
    print(f"Saved multiple-run learned coloring evaluation to: {OUTPUT_CSV}")


if __name__ == "__main__":
    main()