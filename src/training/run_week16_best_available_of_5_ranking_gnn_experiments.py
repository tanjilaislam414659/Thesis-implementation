...
def pairwise_ranking_loss(
    ...
    """
    Pairwise ranking loss for node ordering.

    If target_i > target_j, then prediction_i should also be larger
    than prediction_j. The loss penalizes pairs that violate this order.

    max_pairs is used to keep training fast on larger graphs.
    """
    predictions = predictions.view(-1)
    targets = targets.view(-1)

    num_nodes = targets.numel()
    ...
    target_diff = targets[row_indices] - targets[col_indices]
    valid_pairs = target_diff != 0

    row_indices = row_indices[valid_pairs]
    col_indices = col_indices[valid_pairs]
    target_diff = target_diff[valid_pairs]

    if row_indices.numel() == 0:
        return torch.tensor(0.0, device=predictions.device)

    if row_indices.numel() > max_pairs:
        ...
        target_diff = target_diff[selected]

    desired_sign = torch.sign(target_diff)
    prediction_diff = predictions[row_indices] - predictions[col_indices]

    loss = torch.relu(margin - desired_sign * prediction_diff)
...
OUTPUT_CSV = Path(
    ...
    "week16_best_available_of_5_ranking_training_summary.csv"
)

CHECKPOINT_DIR = Path(
    "results/models/gnn_node_scorer/week16_best_available_of_5_ranking_runs"
)
...
def train_single_run(seed: int) -> dict[str, object]:
    ...
    "pyg_data_week15_best_available_of_5"
)
    ...
    return {
        ...
        "target_ordering": "BEST_AVAILABLE_OF_5_RANKING",
...
def main() -> None:
    print("Week 16 best-available-of-5 ranking-loss GNN experiment")
    print("---------------------------------------")
    print(f"Seeds: {SEEDS}")
    print()
