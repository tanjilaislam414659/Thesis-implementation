...
OUTPUT_CSV = Path(
    "results/tables/gnn_node_scorer/"
    "week16_best_available_of_5_ranking_learned_coloring_evaluation.csv"
)
...
def load_training_run_rows() -> list[dict[str, str]]:
    ...
    if not rows:
        raise ValueError("No rows found in Week 15 training summary.")
...
def main() -> None:
    ...
    print("Week 16 best-available-of-5 ranking-loss learned coloring evaluation")
    print("-------------------------------------------")
    ...
    print()
    print(f"Saved Week 16 best-available-of-5 ranking-loss learned coloring evaluation to: {OUTPUT_CSV}")
