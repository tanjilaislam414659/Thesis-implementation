...
TRAINING_SUMMARY_CSV = Path(
    ...
    "week16_improved_features_best_available_of_5_training_summary.csv"
)

OUTPUT_CSV = Path(
    "results/tables/gnn_node_scorer/"
    "week16_improved_features_best_available_of_5_learned_coloring_evaluation.csv"
)
...
def load_training_run_rows() -> list[dict[str, str]]:
    ...
    if not rows:
        raise ValueError("No rows found in Week 16 improved-feature training summary.")
...
def main() -> None:
    ...
    if not test_graphs:
        raise ValueError("No test graphs found in expanded Week 15 dataset.")

    rows = []

    print("Week 16 improved-feature best-available-of-5 learned coloring evaluation")
    print("-------------------------------------------")
    ...
    for graph_data in test_graphs:
        for training_row in training_rows:
            ...
            result_row = {
                ...
                "target_ordering": "BEST_AVAILABLE_OF_5_IMPROVED_FEATURES",
    ...
    print()
    print(f"Saved Week 16 improved-feature learned coloring evaluation to: {OUTPUT_CSV}")
