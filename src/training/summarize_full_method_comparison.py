"""
Summarize the full Week 12 method comparison table.
...
OUTPUT_CSV = Path(
    "results/tables/gnn_node_scorer/full_method_comparison_summary_jac_pat.csv"
)
...
def method_group(row: dict[str, str]) -> str:
    ...
    if method_family == "colpack":
        return f"ColPack {ordering_name}"

    if method_family == "networkx":
        return f"NetworkX {ordering_name}"

    if method_family == "gnn":
...
def main() -> None:
    ...
    if not rows:
        raise ValueError("No rows found in full method comparison table.")
    ...
    for group_name, group_rows in grouped_rows.items():
        color_counts = [int(row["num_colors"]) for row in group_rows]
        valid_flags = [parse_bool(row["valid"]) for row in group_rows]
    ...
    print("Full method comparison summary")
    print("------------------------------")
    ...
    for row in summary_rows:
        print(
            ...
            f"mean runtime={row['mean_runtime_seconds']:.6f}s"
        )

    print()
    print(f"Saved full summary to: {OUTPUT_CSV}")
