...
OUTPUT_CSV = Path(
    ...
    "week16_normalized_features_edge_separation_training_summary.csv"
)

CHECKPOINT_DIR = Path(
    "results/models/gnn_node_scorer/week16_normalized_features_edge_separation_runs"
)

EDGE_SEPARATION_ALPHA = 0.05
EDGE_SEPARATION_MARGIN = 0.05
...
def edge_separation_loss(
    predictions: torch.Tensor,
    edge_index: torch.Tensor,
    margin: float = EDGE_SEPARATION_MARGIN,
) -> torch.Tensor:
    """
    Penalize adjacent nodes when their predicted ordering scores are too similar.
    """
    ...
    score_differences = torch.abs(scores[source_nodes] - scores[target_nodes])

    return torch.relu(margin - score_differences).mean()
...
def compute_average_loss(
    ...
    with torch.no_grad():
        for data in graphs:
            ...
            mse_loss = loss_fn(predictions.view(-1), data.y.view(-1))
            separation_loss = edge_separation_loss(predictions, data.edge_index)
            loss = mse_loss + EDGE_SEPARATION_ALPHA * separation_loss
...
def train_single_run(seed: int) -> dict[str, object]:
    ...
    return {
        ...
        "target_ordering": "BEST_AVAILABLE_OF_5_NORMALIZED_FEATURES_EDGE_SEPARATION",
...
def main() -> None:
    print("Week 16 normalized-feature edge-separation GNN node scorer experiment")
    print("---------------------------------------")
    print(f"Seeds: {SEEDS}")
    print()
