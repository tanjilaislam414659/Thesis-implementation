...
RESULT_CSV_PATH = (
    ...
    / "tables"
    / "initial_graph_coloring_benchmarks"
    / "week17_heuristic_gap_colpack_benchmark.csv"
)
...
def load_graph_summary():
    rows = {}
    with GRAPH_SUMMARY_PATH.open("r", encoding="utf-8") as f:
...
def main():
    ...
    for txt_path in sorted(OUTPUT_DIR.glob("*.txt")):
        ...
        graph_row = graph_summary.get(graph_id, {})

        known_chromatic_number = graph_row.get("known_chromatic_number")

        rows.append(
            {
                "graph_id": graph_id,
                "family": graph_row.get("family"),
                "construction": graph_row.get("construction"),
                "num_components_joined": graph_row.get("num_components_joined"),
                "base_cycle_size": graph_row.get("base_cycle_size"),
                "source_type": "heuristic_gap_extension",
    ...
    for graph_id in graph_order:
        ...
        graph_row = graph_summary.get(graph_id, {})
        known_chromatic_number = int(graph_row.get("known_chromatic_number"))

        smallest_last_colors = colors_by_ordering.get("SMALLEST_LAST")
        best_gap_from_known = best_colors - known_chromatic_number
        ...
        summary_row = {
            ...
            "smallest_last_gap_from_known": smallest_last_gap_from_known,
            "best_colpack5_gap_from_known": best_gap_from_known,
            "colpack5_stuck_above_known": best_colors > known_chromatic_number,
        }
    ...
    print("Parsed ColPack outputs for Week 17 heuristic-gap extension.")
    print()
    ...
    for row in summary_rows:
        print(
            f"{row['graph_id']} | "
            f"family={row['family']} | "
            f"known={row['known_chromatic_number']} | "
            f"SL={row['SMALLEST_LAST']} | "
            f"best5={row['best_colpack5_colors']} | "
            f"gap={row['best_colpack5_gap_from_known']} | "
