...
def main() -> None:
    ...
    print("Loaded Week 17 symmetry-breaking PyG summary.")
    ...
    for row in summary.itertuples(index=False):
        ...
        if data.x.shape[1] != 25:
            errors.append(
                f"{row.graph_id}: expected 25 symmetry-breaking features, got {data.x.shape[1]}"
            )
        ...
        if float(data.y.min()) < 0 or float(data.y.max()) > 1:
            errors.append(f"{row.graph_id}: y target scores outside [0, 1]")
