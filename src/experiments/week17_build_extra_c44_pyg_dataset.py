...
PYG_SUMMARY_CSV = (
    ...
    / "tables"
    / "gnn_node_scorer"
    / "week17_heuristic_gap_extra_c44_pyg_summary.csv"
)
...
def build_data_for_graph(
    ...
    if actual_node_ids != expected_node_ids:
        raise ValueError(
            f"{graph_id}: node IDs in target file do not match expected node IDs."
        )
...
def main() -> None:
    ...
    print("Extra C44 heuristic-gap PyG dataset built successfully.")
