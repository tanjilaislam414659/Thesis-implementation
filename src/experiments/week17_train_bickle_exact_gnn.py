    ...
    with torch.no_grad():
        for data in graphs:
            ...
            per_graph_parts.append(
                f"{data.graph_id}:{num_colors}/target{target_colors}/colpack5{best_colpack5_colors}"
            )
...
def main() -> None:
    print("Week 17 exact-optimal Bickle GNN training")
    print("-----------------------------------------")
    ...
    for seed in SEEDS:
        ...
        print(
            ...
            f"validation gap target: {row['final_validation_gap_from_target']} | "
            f"validation gap ColPack-5: {row['final_validation_gap_from_colpack5']} | "
            f"test colors: {row['final_test_total_colors']} | "
            f"test gap target: {row['final_test_gap_from_target']} | "
            f"test gap ColPack-5: {row['final_test_gap_from_colpack5']}"
        )
        print()
    ...
    print(f"Saved exact-optimal Bickle training summary to: {OUTPUT_CSV}")
